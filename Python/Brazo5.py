...
def actualizar_posiciones(theta1, theta2, theta_base):
    # Posición del primer eslabón (movimiento en Y)
    y1 = L1 * np.cos(theta1)
    z1 = L1 * np.sin(theta1)
    
    # Posición del segundo eslabón respecto al primero (movimiento en X)
    x2_local = L2 * np.cos(theta2)
    z2_local = z1 + L2 * np.sin(theta2)
    
    # Aplicar la rotación de la base en torno al eje Y al segundo eslabón
    x2 = x2_local * np.cos(theta_base)
    y2 = y1  # Mantiene la altura del primer eslabón
    z2 = x2_local * np.sin(theta_base) + z2_local
    
    return (0, y1, z1), (x2, y2, z2)
...
ax.set_ylabel('Y')
ax.set_zlabel('Z')
ax.set_title('Brazo Robótico 2 Animado')

# Definir movimientos en secuencia
movimientos = [
    {'theta1': (0, 45), 'theta2': (0, 30), 'theta_base': (0, 90)},
    {'theta1': (45, 90), 'theta2': (30, 90), 'theta_base': (90, 90)}
]
...
def update(frame):
    ...
    line1.set_data([0, 0], [0, y1])
    line1.set_3d_properties([0, z1])
    
    line2.set_data([0, x2], [y1, y2])
